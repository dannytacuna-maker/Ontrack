...
for (let i = 1; i <= doc.numPages; i++) {
  ...
  console.log("page", i, "images", [...names]);
  for (const name of names) {
    try {
      const obj = await getObj(page, name);
      console.log(" ", name, obj && { w: obj.width, h: obj.height, kind: obj.kind, len: obj.data?.length, keys: Object.keys(obj) });
      if (obj?.data && obj.width && obj.height) {
        const out = `img-p${i}-${String(name).replace(/[^a-z0-9_-]/gi,"_")}.png`;
